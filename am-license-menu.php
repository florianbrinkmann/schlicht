	...
	class AM_License_Menu {
		...
		public function __construct( $file, $software_title, $software_version, $plugin_or_theme, $api_url, $text_domain, $extra ) {
			...
			if ( is_admin() ) {
				...
				$this->ame_settings_menu_title         = $this->software_title . __( ' Activation', 'schlicht' );
				$this->ame_settings_title              = $this->software_title . __( ' Key Activation', 'schlicht' );
				$this->ame_menu_tab_activation_title   = __( 'Key Activation', 'schlicht' );
				$this->ame_menu_tab_deactivation_title = __( 'Key Deactivation', 'schlicht' );
		...
		public function register_menu() {
			add_options_page( $this->software_title . __( ' Activation', 'schlicht' ), $this->software_title . __( ' Activation', 'schlicht' ), 'manage_options', $this->ame_activation_tab_key, array(
				$this,
				'config_page'
			) );
		}
		...
		public function load_settings() {
			...
			// API Key
			add_settings_section( $this->ame_api_key, __( 'Key Activation', 'schlicht' ), array(
				$this,
				'wc_am_api_key_text'
			), $this->ame_activation_tab_key );
			add_settings_field( 'status', __( 'Key Status', 'schlicht' ), array(
				$this,
				'wc_am_api_key_status'
			), $this->ame_activation_tab_key, $this->ame_api_key );
			add_settings_field( $this->ame_api_key, __( 'Key', 'schlicht' ), array(
				$this,
				'wc_am_api_key_field'
			), $this->ame_activation_tab_key, $this->ame_api_key );
			add_settings_field( $this->ame_activation_email, __( 'Email', 'schlicht' ), array(
				$this,
			...
			add_settings_section( 'deactivate_button', __( 'Deactivation', 'schlicht' ), array(
				$this,
				'wc_am_deactivate_text'
			), $this->ame_deactivation_tab_key );
			add_settings_field( 'deactivate_button', __( 'Deactivate Key', 'schlicht' ), array(
				$this,
			...
			if ( $_REQUEST['option_page'] != $this->ame_deactivate_checkbox ) {
				if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $api_email == '' || $checkbox_status == 'on' || $current_api_key != $api_key ) {
					...
					if ( $activate_results['activated'] === true && ! empty( $this->ame_activated_key ) ) {
						add_settings_error( 'activate_text', 'activate_msg', sprintf( __( '%s activated. ', 'schlicht' ), esc_attr( $this->software_title ) ) . "{$activate_results['message']}.", 'updated' );
					...
					if ( $activate_results == false && ! empty( $this->ame_options ) && ! empty( $this->ame_activated_key ) ) {
						add_settings_error( 'api_key_check_text', 'api_key_check_error', __( 'Connection failed to the License Key API server. Try again later.', 'schlicht' ), 'error' );
		...
		public function replace_license_key( $current_api_key ) {
			...
			add_settings_error( 'not_deactivated_text', 'not_deactivated_error', __( 'The Key could not be deactivated. Use the Key Deactivation tab to manually deactivate the Key before activating a new Key.', 'schlicht' ), 'updated' );
			...
			if ( $options == 'on' && $activation_status == 'Activated' && $this->ame_options[ $this->ame_api_key ] != '' && $this->ame_options[ $this->ame_activation_email ] != '' ) {
				...
				if ( $activate_results['deactivated'] === true ) {
					...
					if ( ! empty( $this->ame_activated_key ) ) {
						...
						add_settings_error( 'wc_am_deactivate_text', 'deactivate_msg', __( 'Key deactivated. ', 'schlicht' ) . "{$activate_results['activations_remaining']}.", 'updated' );
					}
		...
		public function expired_license_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The license key for %s has expired. You can reactivate or purchase a license key from your account <a href="%s" target="_blank">dashboard</a>.', 'schlicht' ) . '</p></div>', esc_attr( $this->ame_software_product_id ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function on_hold_subscription_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s is on-hold. You can reactivate the subscription from your account <a href="%s" target="_blank">dashboard</a>.', 'schlicht' ) . '</p></div>', esc_attr( $this->ame_software_product_id ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function cancelled_subscription_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s has been cancelled. You can renew the subscription from your account <a href="%s" target="_blank">dashboard</a>. A new license key will be emailed to you after your order has been completed.', 'schlicht' ) . '</p></div>', esc_attr( $this->ame_software_product_id ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function expired_subscription_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s has expired. You can reactivate the subscription from your account <a href="%s" target="_blank">dashboard</a>.', 'schlicht' ) . '</p></div>', esc_attr( $this->ame_software_product_id ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function suspended_subscription_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s has been suspended. You can reactivate the subscription from your account <a href="%s" target="_blank">dashboard</a>.', 'schlicht' ) . '</p></div>', esc_attr( $this->ame_software_product_id ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function pending_subscription_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s is still pending. You can check on the status of the subscription from your account <a href="%s" target="_blank">dashboard</a>.', 'schlicht' ) . '</p></div>', esc_attr( $this->ame_software_product_id ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function trash_subscription_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s has been placed in the trash and will be deleted soon. You can purchase a new subscription from your account <a href="%s" target="_blank">dashboard</a>.', 'schlicht' ) . '</p></div>', esc_attr( $this->ame_software_product_id ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function no_subscription_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'A subscription for %s could not be found. You can purchase a subscription from your account <a href="%s" target="_blank">dashboard</a>.', 'schlicht' ) . '</p></div>', esc_attr( $this->ame_software_product_id ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function download_revoked_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'Download permission for %s has been revoked possibly due to a license key or subscription expiring. You can reactivate or purchase a license key from your account <a href="%s" target="_blank">dashboard</a>.', 'schlicht' ) . '</p></div>', esc_attr( $this->ame_software_product_id ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function no_activation_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( '%s has not been activated. Go to the settings page and enter the license key and license email to activate %s.', 'schlicht' ) . '</p></div>', esc_attr( $this->ame_software_product_id ), esc_attr( $this->ame_software_product_id ) );
		}
		...
		public function switched_subscription_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'You changed the subscription for %s, so you will need to enter your new API License Key in the settings page. The License Key should have arrived in your email inbox, if not you can get it by logging into your account <a href="%s" target="_blank">dashboard</a>.', 'schlicht' ) . '</p></div>', esc_attr( $this->ame_software_product_id ), esc_url( $this->ame_renew_license_url ) );
		}
